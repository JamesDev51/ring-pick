...
export function RingCard({ src, attributes, onSelect, selected = false, disabled = false, testId }: Props) {
  ...
  function click(){ if(failed){setFailed(false);setLoaded(false);setRetry(v=>v+1);return;} if(!loaded)return; onSelect(); }
  const label = failed ? '다시 불러오기' : !loaded ? '이미지 불러오는 중' : selected ? '선택했어요' : '이쪽이 더 좋아요';
  return <button data-testid={testId} className={`ring-card ${selected ? 'is-selected' : ''}`} type="button" onClick={click} disabled={disabled} aria-disabled={disabled || (!loaded && !failed)} aria-label={failed?'반지 이미지를 다시 불러오기':`반지 선택: ${aria}`}>
    ...
    <span className="ring-pick-label">{label}</span>
  </button>;
}
